...
void tree_destroy(struct tree_t *tree){
  ...
  //NAO FAZ SENTIDO O ! DENTRO DOS IFs

  if(tree != NULL){
    entry_destroy(tree->node);
    tree_destroy(tree->left);
    tree_destroy(tree->right);
    //if(tree->node) entry_destroy(tree->node);
    free(tree);
  }
...
int tree_put(struct tree_t *tree, char *key, struct data_t *value){
  ...
  if (tree->node == NULL){
    ...
    if (result == 0){
      ...
      //DIREITA
      if(tree->right == NULL) {
        //memcmp(tree->right, tree_create(), sizeof(struct tree_t));

        //struct tree_t *toAdd = tree_create();

        //tree->right = (tree_t) malloc(sizeof(struct tree_t));
        tree->right = tree_create();    //LEAKSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS
...
char **tree_get_keys(struct tree_t *tree){
  ...
  if(tree == NULL || tree->node->key == NULL){
    return NULL;
  }
