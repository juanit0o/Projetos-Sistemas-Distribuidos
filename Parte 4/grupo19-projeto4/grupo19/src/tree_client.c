...
int main(int argc, char **argv) {
	...
	if (argc != 2) {
		printf("[ERROR] Invalid input! Try ./tree_client <ipZoo>:<portZoo>\n");
		return -1;
	}
	...
	for (;;) {
		...
		if(rtreePrimary != NULL && rtreeBackup != NULL){
			...
			if (strlen(message_client) <= 0 || message_client == NULL || contador == 1) {
				...
				if ((msg_dup[5] == ' ' && msg_dup[6] == 0)
					...
					if(rtree_put(rtreePrimary, entry_put) != 0) {
					...
					if (counter != 1) {
						...
						struct data_t *data_result = rtree_get(rtreeBackup, key);
					...
					if (counter != 1 || (msg_dup[3] == ' ' && msg_dup[4] == 0)) {
						...
						if (rtree_del(rtreePrimary, key) != 0) {
					...
					int size = rtree_size(rtreeBackup);
					if (size < 0) {
					...
					int height = rtree_height(rtreeBackup);
					if (height < 0) {
					...
					keys = rtree_get_keys(rtreeBackup);
					...
					if (keys != NULL && keys[0] != NULL) {
						...
						if (counter != 1) {
							...
							if ((!(op_n = strdup(strtok(NULL, " "))) && (atoi(op_n)%10 != 0)) ||
								...
								int verifica = rtree_verify(rtreeBackup, atoi(op_n));
			...
			if (strcmp(strtok(strdup(message_client), " "), "quit") == 0) {
				printf("Disconnecting...\n");
				exit(-1);
			} else if (strcmp(strtok(strdup(message_client), " "), "help") == 0) {
				printf("Possible inputs:\n");
