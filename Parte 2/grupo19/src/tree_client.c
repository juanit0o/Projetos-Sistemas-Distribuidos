		...
		if (strlen(message_client) <= 0 || message_client == NULL || contador == 1) {
			...
			if ((msg_dup[5] == ' ' && msg_dup[6] == 0)
				...
				if(rtree_put(rtree, entry_put) == 0) {
					printf("Success! Inserted element in tree %s!\n", key);
				} else {
					printf("[ERROR] Couldn't insert element in tree %s!\n", key);
				}
				...
				if (counter != 1 || (msg_dup[3] == ' ' && msg_dup[4] == 0)) {
					...
					if (rtree_del(rtree, key) == 0) {
						printf("Success! Deleted element from tree: %s!\n", key);
					} else {
						printf("[ERROR] Couldn't delete element from tree: %s!\n", key);
					}
					free(msg_dup);
				}
